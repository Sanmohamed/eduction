...
export default function MainLayout() {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-slate-50 dark:bg-[#000000] overflow-x-hidden transition-colors duration-300">

   
      <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-[#101622]/80 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-3">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <h2 className="text-slate-900 dark:text-slate-50 text-lg font-bold tracking-tight">
              EduPlatform
            </h2>
          </div>
          ...
          <nav className="flex flex-wrap items-center gap-1 text-sm font-medium">
            ...
            <NavLink to="/wishlist" className={linkClass}>
              Wishlist
              {wishlist?.length > 0 && (
                <span className="ml-1 rounded-full bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
                  {wishlist.length}
                </span>
              )}
            </NavLink>
            ...
            {user ? (
              <>
                ...
                <button
                  onClick={logout}
                  className="rounded-xl bg-slate-900 dark:bg-slate-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 dark:hover:bg-slate-600"
                >
                  Logout
                </button>
              </>
