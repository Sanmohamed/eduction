...
const { mergeCartRequest } = useCart()
...
export default function LoginPage() {
  ...
  // Toggle .dark on <html> — app.css handles theming via CSS variables
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden transition-colors duration-300">
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header className="relative z-10 flex items-center justify-between px-6 py-3.5 shadow-sm bg-white dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-[9px] bg-blue-600 flex items-center justify-center text-white font-semibold text-sm shadow-md shadow-blue-600/30 flex-shrink-0">
            E
          </div>
          <span className="font-semibold text-[15px] tracking-tight hidden sm:block text-slate-900 dark:text-slate-100">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-200 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:border-blue-400 hover:text-blue-600"
          >
            <ChatIcon />
      ...
      {/* ── Main ───────────────────────────────────────────────────────────── */}
      <main className="relative z-[1] flex items-center justify-center px-4 py-12 min-h-[calc(100vh-57px)]">
        <div className="w-full max-w-[440px] animate-slide-up">

          {/* Hero */}
          <div className="text-center mb-7">
            <h1 className="text-3xl font-bold tracking-tight mb-2 leading-tight text-slate-900 dark:text-white">
              Welcome back to{' '}
          ...
          <div className="rounded-2xl overflow-hidden shadow-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              ...
              <div className="flex rounded-xl p-1 gap-1 border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-[13px] font-medium transition-all duration-200 ${userType === val
            ...
            <div className="flex px-5 mt-4 border-b border-slate-200 dark:border-slate-800">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() => setTab(val)}
                  className={`flex-1 py-3 text-[13.5px] font-medium border-b-2 transition-all duration-200 ${tab === val
            ...
            <div className="p-5">
              {tab === 'login'
                ? <LoginPanel key={`login-${userType}`} userType={userType} />
